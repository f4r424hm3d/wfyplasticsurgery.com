        ...
        <div class="col-12">
          <div class="page-title-box d-sm-flex align-items-center justify-content-between">
            <h4 class="mb-sm-0 font-size-18">
              {{ $page_title }} <span class="text-danger">[ {{ $department->department_name }} ]
            </h4>
          ...
          <div class="card">
            ...
            <div class="card-body" id="tblCDiv">
              <form id="{{ $ft == 'add' ? 'dataForm' : 'editForm' }}" {{ $ft == 'edit' ? 'action=' . $url : '' }}
                ...
                @endif
                @if ($ft == 'edit')
                  <a href="{{ aurl($page_route . '/' . $department->id) }}" class="btn btn-sm btn-info "><i
                      class="ti-trash"></i>
                    Cancel</a>
                @endif
  ...
  <script>
    ...
    $(document).ready(function() {
      ...
      $('#filterForm').on('submit', function(event) {
        ...
        setTimeout(() => {
          $(".errSpan").text('');
          var department_id = $("#f_hospital_id").val();
          //alert(department_id);
          getData('1', department_id);
